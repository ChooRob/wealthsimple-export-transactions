    ...
    for (const transaction of transactions) {
      ...
      switch (type) {
        ...
        case "WITHDRAWAL/E_TRANSFER":
          ...
          notes = `Sent INTERAC e-Transfer to ${transaction.eTransferName}`;
          break;
        case "DEPOSIT/E_TRANSFER":
          payee = transaction.eTransferEmail;
          notes = `Received INTERAC e-Transfer from ${transaction.eTransferName}`;
          break;
        ...
        case "P2P_PAYMENT/SEND_RECEIVED":
          ...
          notes = `Received WealthSimple Cash transfer from ${transaction.p2pHandle}`
          if (transaction.p2pMessage) {
              notes += ` with note : ${transaction.p2pMessage}`;
          }
          break;
        case "P2P_PAYMENT/SEND":
          payee = transaction.p2pHandle;
          notes = `Sent WealthSimple Cash transfer to ${transaction.p2pHandle}`
          if (transaction.p2pMessage) {
              notes += ` with note : ${transaction.p2pMessage}`;
          }
          break;
